...
uint8_t can_init(uint8_t tsjw, uint8_t tbs2, uint8_t tbs1, uint16_t brp, uint8_t mode)
{
    ...
    sys_gpio_set(CAN_RX_GPIO_PORT, CAN_RX_GPIO_PIN,
                 SYS_GPIO_MODE_AF, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);   /* CAN_RX脚 模式设置 */

    sys_gpio_af_set(CAN_TX_GPIO_PORT, CAN_TX_GPIO_PIN, CAN_TX_GPIO_AF);     /* TX脚 复用功能选择, 必须设置正确 */
    sys_gpio_af_set(CAN_RX_GPIO_PORT, CAN_RX_GPIO_PIN, CAN_RX_GPIO_AF);     /* RX脚 复用功能选择, 必须设置正确 */
