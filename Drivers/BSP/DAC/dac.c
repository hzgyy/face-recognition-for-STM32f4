 ...
 *              DAC在输出buffer关闭的时候, 输出建立时间: tSETTLING = 3us (F407数据手册有写)
...
void dac_init(uint8_t outx)
{
    ...
    RCC->AHB1ENR |= 1 << 0;;        /* 使能DAC OUT1/2的IO口时钟(都在PA口,PA4/PA5) */
    RCC->APB1ENR |= 1 << 29;        /* 使能DAC的时钟 */
...
void dac_dma_wave_init(uint8_t outx, uint32_t par, uint32_t mar)
{
    ...
    if (outx == 1)
    {
        dma_streamx = DMA1_Stream5;     /* OUT1对应DMA1_Stream5 */
    }
    else
    {
        dma_streamx = DMA1_Stream6;     /* OUT2对应DMA1_Stream6 */
...
/**
 * @brief       DAC DMA使能波形输出
 *   @note      TIM7的输入时钟频率(f)来自APB1, f = 42M * 2 = 84Mhz.
...
void dac_dma_wave_enable(uint8_t outx, uint16_t ndtr, uint16_t arr, uint16_t psc)
{
    ...
    if (outx == 1)
    {
        dma_enable(DMA1_Stream5, ndtr); /* 重新启动DMA2_Channel3传输 */
    }
    else
    {
        dma_enable(DMA1_Stream6, ndtr); /* 重新启动DMA2_Channel4传输 */
    }
