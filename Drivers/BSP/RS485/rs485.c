 ...
 *              串口1 的时钟源来自: PCLK2 = 84Mhz
 *              串口2 - 5 的时钟源来自: PCLK1 = 42Mhz
...
void rs485_init(uint32_t sclk, uint32_t baudrate)
{
    ...
    sys_gpio_set(RS485_RX_GPIO_PORT, RS485_RX_GPIO_PIN,
                 SYS_GPIO_MODE_AF, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);   /* 串口RX 脚 模式设置 */

    sys_gpio_af_set(RS485_TX_GPIO_PORT, RS485_TX_GPIO_PIN, RS485_TX_GPIO_AF);   /* TX脚 复用功能选择, 必须设置正确 */
    sys_gpio_af_set(RS485_RX_GPIO_PORT, RS485_RX_GPIO_PIN, RS485_RX_GPIO_AF);   /* RX脚 复用功能选择, 必须设置正确 */

    temp = (sclk * 1000000 + baudrate / 2) / baudrate;  /* 得到BRR, 采用四舍五入计算 */
    
    /* 波特率设置 */
    RS485_UX->BRR = temp;       /* 波特率设置@OVER8 = 0 */
    RS485_UX->CR1 = 0;          /* 清零CR1寄存器 */
    RS485_UX->CR1 |= 0 << 12;   /* 设置M = 0, 选择8位字长 */
    RS485_UX->CR1 |= 0 << 15;   /* 设置OVER8 = 0, 16倍过采样 */
    RS485_UX->CR1 |= 1 << 3;    /* 串口发送使能 */

#if RS485_EN_RX  /* 如果使能了接收 */
    /* 使能接收中断 */
    RS485_UX->CR1 |= 1 << 2;    /* 串口接收使能 */
    RS485_UX->CR1 |= 1 << 5;    /* 接收缓冲区非空中断使能 */
